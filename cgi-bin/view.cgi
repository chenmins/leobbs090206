...
if ($addmefile == 1)
{
	...
	if ($up_ext eq "gif" || $up_ext eq "jpg" || $up_ext eq "png" || $up_ext eq "bmp")
	{
		...
		else
		{
	        ...
	        $addme .= qq(<img src=$imagesurl/images/none.gif whidth=0 height=5><BR><span style=CURSOR:hand onclick=loadThreadFollow($inforum,$intopic,1,'$up_ext')><img id=followImg1 src=$imagesurl/images/cat.gif width=9 loaded=no nofollow="cat.gif" valign=absmiddle> 按此查看图片详细信息<table cellpadding=0 class=ts1 cellspacing=0 width=50% id=follow1 style=DISPLAY:none><tr><td id=followTd1><DIV class=ts onclick=loadThreadFollow($inforum,$intopic,1,'$up_ext')>正在读取此图片的详细信息，请稍候 ...</DIV></td></tr></table></span><BR><BR>);
	}
	elsif ($up_ext eq "swf")
	{
...
for ($i = 1; $i < @threads; $i++)
{
	...
	if ($up_ext eq "gif" || $up_ext eq "jpg" || $up_ext eq "png" || $up_ext eq "bmp")
	{
		...
		else
		{
	        ...
	        $addme .= qq(<img src=$imagesurl/images/none.gif whidth=0 height=5><BR><span style=CURSOR:hand onclick=loadThreadFollow($inforum,$intopic,$rnrn,'$up_ext')><img id=followImg$rnrn src=$imagesurl/images/cat.gif width=9 loaded=no nofollow="cat.gif" valign=absmiddle> 按此查看图片详细信息<table cellpadding=0 class=ts1 cellspacing=0 width=50% id=follow$rnrn style=DISPLAY:none><tr><td id=followTd$rnrn><DIV class=ts onclick=loadThreadFollow($inforum,$intopic,$rnrn,'$up_ext')>正在读取此图片的详细信息，请稍候 ...</DIV></td></tr></table></span><BR><BR>);
	}
	elsif ($up_ext eq "swf")
	{
		if ($arrawpostflash eq "on")
